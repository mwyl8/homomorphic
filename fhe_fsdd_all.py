...
def fhe_query_encrypted_fsdd_collection(client, collection_name, limit=1000):
    ...
    # Map from ID to vector (match your style)
    ...
    # Encrypt query vector element-wise (rounded to ints), like your FMA script
    enc_query = [ts.ckks_vector(context, [int(round(x))]) for x in query_vector]

    process = psutil.Process(os.getpid())
    num_cpus = psutil.cpu_count(logical=True)

    # overall collection tracking
    total_time_sum = 0.0
    processed = 0
    ...
    for i in range(limit):
        ...
        # Encrypt db vector element-wise (rounded ints) like your FMA script
...
def fhe_query_encrypted_all_fsdd():
    ...
    for collection_name in collections:
        fhe_query_encrypted_fsdd_collection(client, collection_name, limit=1000)


if __name__ == "__main__":
    fhe_query_encrypted_all_fsdd()
