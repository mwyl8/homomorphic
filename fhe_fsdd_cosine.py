...
def fhe_cosine_fsdd_collection(client, collection_name, limit=1000):
    ...
    # Encrypt query element-wise (same as yours)
    enc_query = [ts.ckks_vector(context, [int(round(x))]) for x in query_vector]

    # Precompute query norm in plaintext (HE-friendly baseline)
    ...
    for i in range(limit):
        ...
        # plaintext norm of db (HE-friendly baseline)
        ...
        # cosine = dot * inv_norm (inv_norm is plaintext scalar)
...
def fhe_cosine_all_fsdd():
    ...
    for collection_name in collections:
        fhe_cosine_fsdd_collection(client, collection_name, limit=1000)
