...
@track_performance
def compute_dot_products(query_vector, database_vectors):
...
for collection_name in collections:
    print(f"\n=== Running for collection: {collection_name} ===")
    try:
        ...
        if len(results) < 2:
            print(f"Skipping {collection_name}: not enough vectors.")
            continue
        ...
        compute_dot_products(query_vector, database_vectors)
