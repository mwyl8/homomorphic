...
# === Core AHE Logic (DB encrypted) ===
def ahe_db_encrypted():
    # === 1. Connect to Milvus and fetch vectors ===
    client = MilvusClient("milvus_demo.db")
    collection_name = "fsdd_audio_vector_256"
    ...
    # === 3. TenSEAL context and encrypted DB ===
    context = ts.context(
        ts.SCHEME_TYPE.CKKS,
    ...
    # Encrypt each element of each DB vector individually
    encrypted_db_vectors = []
    for db_vec in database_vectors:
        encrypted_db_vectors.append([ts.ckks_vector(context, [v]) for v in db_vec])
    ...
    @track_performance
    def doing_stuff():
        total = min(1000, len(encrypted_db_vectors))
        time_spent = 0.0
        for idx, db_vec in enumerate(encrypted_db_vectors[:total], start=1):
            enc_dot = ts.ckks_vector(context, [0.0])
            start = perf_counter()
            for i in range(dim):
                y_i = query_vector_int[i]
                partial = fast_repeated_add(db_vec[i], y_i)
                enc_dot += partial
